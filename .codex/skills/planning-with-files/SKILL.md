...
### 7. Clean Up Temporary Planning Files
When the task is done and key info is captured in the final response or committed docs, remove the temporary planning files to keep git clean:

- Delete `task_plan.md`, `findings.md`, and `progress.md` in the project root
- If the user explicitly wants to keep them, move them to a non-tracked location or add them to `.gitignore`
